...
@testset "defensive shape and alias checks" begin
    ...
    @test_throws ArgumentError FourierGrid((4,), (Float32(1e-38),))
    @test_throws DimensionMismatch FourierGrid((8, 8), (T(1),))
    @test_throws DimensionMismatch FourierGrid((8,), (T(1), T(1)))
    tiny_singleton = FourierGrid((1,), (nextfloat(0.0f0),))
    @test tiny_singleton.dx == (nextfloat(0.0f0),)
    @test tiny_singleton.kfull == (Float32[0],)
    @test deriv(ones(Float32, 1), tiny_singleton, 1) == Float32[0]
    ...
    offset_out = OffsetSbpMatrix(similar(f), 0, 1)
    @test_throws DimensionMismatch deriv!(out, offset_f, g, 1)
    @test_throws DimensionMismatch deriv!(offset_out, f, g, 1)
    @test_throws DimensionMismatch gradient!((offset_out, similar(f)), f, g)
    @test_throws DimensionMismatch gradient!((similar(f), similar(f)), offset_f, g)
    @test_throws DimensionMismatch divergence!(offset_out, (f, f), g)
    @test_throws DimensionMismatch divergence!(similar(f), (offset_f, f), g)
    @test_throws DimensionMismatch laplacian!(similar(f), offset_f, g)
    @test_throws DimensionMismatch laplacian!(offset_out, f, g)
    @test_throws DimensionMismatch exp_filter!(offset_f, g)
    @test_throws DimensionMismatch dealias_two_thirds!(offset_f, g)
    ...
    @test_throws DimensionMismatch curl!((offset_out, B[2], B[3]), A, g)
    @test_throws DimensionMismatch curl!(B, (offset_f, A[2], A[3]), g)
    ...
    @test_throws ArgumentError SBP1D(3, nextfloat(0.0f0))
    s = SBP1D(4, T(1))
    ...
    @test_throws ArgumentError FourierDerivYWorkspace(2, 4, Float32(1e-38))
    tiny_y = FourierDerivYWorkspace(2, 1, nextfloat(0.0f0))
    @test tiny_y.ky == Float32[0]
